...
#define MUTEX_RESET(m) ((m)?sem_init(__MM__(m), 0, 1):-1)
...
#define MUTEX_WAIT(m) ((m)?sem_wait(__MM__(m)):-1)
#define MUTEX_TIMEDWAIT(m, ts) ((m)?sem_timedwait(__MM__(m), &ts):-1)
#define MUTEX_SIGNAL(m) ((m)?sem_post(__MM__(m)):-1)
#define MUTEX_DESTROY(m) do{if(m){sem_destroy(__MM__(m));free(m);}}while(0)
#else
