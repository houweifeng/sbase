...
int evkqueue_add(EVBASE *evbase, EVENT *event)
{
	...
	if(evbase && event && evbase->ev_changes && evbase->evs)
	{
		...
		if(event->ev_fd > evbase->max_fd)
			evbase->max_fd = event->ev_fd;
		return 0;
	}
	return -1;
}
...
void evkqueue_loop(EVBASE *evbase, short, timeval *tv)
{
	...
	if(evbase)
	{
		...
		n = kevent(evbase->fd, evbase->ev_changes, evbase->max_fd, evbase->evs, evbase->max_fd, &ts);	
		for(; i < n; i++)
		{
			kqev = &(evbase->evs[i]);
