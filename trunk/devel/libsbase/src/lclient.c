...
#define LCLIENT_LOG      "/tmp/lclient_access_log"
#define LCLIENT_EVLOG      "/tmp/lclient_evbase_log"
SBASE *sbase = NULL;
...
int sbase_initialize(SBASE *sbase, char *conf)
{
	...
	/* LCLIENT */
	fprintf(stdout, "Parsing LCLIENT...\n");
	...
	n = iniparser_getint(dict, "LCLIENT:inet_family", 0);
	/* INET protocol family */
	n = iniparser_getint(dict, "LCLIENT:inet_family", 0);
	switch(n)
	{
	...
	/* socket type */
	n = iniparser_getint(dict, "LCLIENT:socket_type", 0);
	switch(n)
	{
		case 0:
	...
	service->name = iniparser_getstr(dict, "LCLIENT:service_name");
	service->ip = iniparser_getstr(dict, "LCLIENT:service_ip");
	if(service->ip && service->ip[0] == 0 )
		service->ip = NULL;
	service->port = iniparser_getint(dict, "LCLIENT:service_port", 80);
	service->working_mode = iniparser_getint(dict, "LCLIENT:working_mode", WORKING_PROC);
	service->max_procthreads = iniparser_getint(dict, "LCLIENT:max_procthreads", 1);
	service->sleep_usec = iniparser_getint(dict, "LCLIENT:sleep_usec", 100);
    /* connections number */
	service->connections_limit = iniparser_getint(dict, "LCLIENT:connections_limit", 32);
	logfile = iniparser_getstr(dict, "LCLIENT:logfile");
	if(logfile == NULL)
		logfile = LCLIENT_LOG;
	service->logfile = logfile;
	logfile = iniparser_getstr(dict, "LCLIENT:evlogfile");
	service->evlogfile = logfile;
	service->max_connections = iniparser_getint(dict, "LCLIENT:max_connections", MAX_CONNECTIONS);
	service->packet_type = PACKET_DELIMITER;
	service->packet_delimiter = iniparser_getstr(dict, "LCLIENT:packet_delimiter");
	...
	*s++ = 0;
	...
	service->buffer_size = iniparser_getint(dict, "LCLIENT:buffer_size", BUF_SIZE);
