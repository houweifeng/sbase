...
{                                                                                           \
    ...
    {                                                                                       \
        ...
        DEBUG_LOGGER(conn->logger, "Read %d bytes ndata:%d left:%lld to chunk from %s:%d"   \
                " on %s:%d via %d", n, CK_NDATA(conn->chunk), LL(CK_LEFT(conn->chunk)),     \
...
int conn_read_handler(CONN *conn)
{
    ...
    if(conn)
    {
        ...
        DEBUG_LOGGER(conn->logger, "Received %d bytes nbuffer:%d/%d  left:%d data total %lld "
                ...
                MB_LEFT(conn->buffer), conn->recv_data_total, conn->remote_ip, conn->remote_port, 
                conn->local_ip, conn->local_port, conn->fd);
        //for proxy
        if(conn->session.packet_type & PACKET_PROXY)
        {
...
int conn_packet_handler(CONN *conn)
{
    ...
    if(conn && conn->session.packet_handler)
    {
        ...
        {
            DEBUG_LOGGER(conn->logger, "Reset packet_handler(%p) buffer:[%d/%d] on %s:%d via %d", conn->session.packet_handler, MB_LEFT(conn->buffer), MB_SIZE(conn->buffer), conn->remote_ip, conn->remote_port, conn->fd);
            SESSION_RESET(conn);
        }
    }
    return ret;
                ...
                && MB_NDATA(conn->buffer) > 0)
        {
            ...
            if(n > 0)
            {
                DEBUG_LOGGER(conn->logger, "Filled  %d byte(s) left:%lld to chunk from buffer "
                        "to %s:%d on conn[%s:%d] via %d", n, LL(CK_LEFT(conn->chunk)),
                        conn->remote_ip, conn->remote_port, conn->local_ip, 
                        conn->local_port, conn->fd);
                ret = 0;
            }
